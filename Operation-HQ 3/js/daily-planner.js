...
const DailyPlanner = {
  ...
  async getStoredPlan() {
    const saved = await chrome.storage.local.get([
      "hq_daily_plan_date", "hq_daily_plan_tasks", "hq_daily_plan_premortem", "hq_daily_plan_rationale", "hq_daily_plan_future_v1",
    ]);
    const prepared = saved.hq_daily_plan_future_v1;
    if (prepared?.date === this.todayKey()) {
      if (saved.hq_daily_plan_date && saved.hq_daily_plan_date < this.todayKey() && (saved.hq_daily_plan_tasks || []).length) {
        await this.archivePlan(saved.hq_daily_plan_date, saved.hq_daily_plan_tasks);
      }
      await chrome.storage.local.set({
        hq_daily_plan_date: prepared.date,
        hq_daily_plan_tasks: prepared.tasks || [],
        hq_daily_plan_premortem: prepared.preMortem || null,
        hq_daily_plan_rationale: prepared.rationale || "",
      });
      await chrome.storage.local.remove("hq_daily_plan_future_v1");
      return { date: prepared.date, tasks: prepared.tasks || [], preMortem: prepared.preMortem || null, rationale: prepared.rationale || "", promoted: true };
    }
    if (prepared?.date && prepared.date < this.todayKey()) await chrome.storage.local.remove("hq_daily_plan_future_v1");
    return { date: saved.hq_daily_plan_date || null, tasks: saved.hq_daily_plan_tasks || [], preMortem: saved.hq_daily_plan_premortem || null, rationale: saved.hq_daily_plan_rationale || "", promoted: false };
  },
  ...
  async archivePlan(date, tasks) {
    const plannedMinutes = tasks.reduce((sum, task) => sum + (task.minutes || 0), 0);
    const completedMinutes = tasks.filter(task => task.done).reduce((sum, task) => sum + (task.minutes || 0), 0);
  ...
  async archiveYesterdayIfNeeded() {
    const saved = await chrome.storage.local.get(["hq_daily_plan_date", "hq_daily_plan_tasks"]);
    const date = saved.hq_daily_plan_date;
    const tasks = saved.hq_daily_plan_tasks || [];
    if (!date || date === this.todayKey() || !tasks.length) return;
    await this.archivePlan(date, tasks);
  },
  ...
  async buildContext(targetDate = new Date()) {
    const date = new Date(targetDate);
    date.setHours(12, 0, 0, 0);
    const targetKey = this.dateKey(date);
    const { hq_tasks, hq_calendar_events, hq_schedule_profiles, hq_active_schedule_profile } = await chrome.storage.local.get(["hq_tasks", "hq_calendar_events", "hq_schedule_profiles", "hq_active_schedule_profile"]);
    ...
    const fallbackDayKeys = ["sun", "mon", "tue", "wed", "thu", "fri", "sat"];
    const dayKey = typeof SCHEDULE_DAY_KEYS !== "undefined" ? SCHEDULE_DAY_KEYS[dow] : fallbackDayKeys[dow];
    const storedProfile = hq_schedule_profiles?.[hq_active_schedule_profile] || null;
    const activeProfile = typeof Schedule !== "undefined" && Schedule.active ? Schedule.active : storedProfile;
    const rawBlocks = typeof Schedule !== "undefined" && activeProfile && dayKey
      ? Schedule.blocksFor(activeProfile, dayKey, date)
      : (activeProfile?.days?.[dayKey] || []);
    const scheduleBlocks = rawBlocks.map(block => `${block.s}–${block.e} ${block.t}${block.phasePriority ? ` [${block.phasePriority}]` : ""}`);
    const rhythmToday = scheduleBlocks.length ? scheduleBlocks : this.WEEKLY_RHYTHM.filter(r => r.day === dow).map(r => r.label);
    const phase = typeof Schedule !== "undefined"
      ? Schedule.activePhase?.(activeProfile, date)
      : activeProfile?.phases?.find(candidate => targetKey >= candidate.start && targetKey <= candidate.end) || null;
    const todayEvents = (hq_calendar_events || {})[targetKey] || [];
    ...
    const examTargets = typeof ExamCountdown !== "undefined" && targetKey === this.todayKey() ? await ExamCountdown.todaysTargets() : [];

    return { openTasks, rhythmToday, phase, dailyNonNegotiables: this.DAILY_NON_NEGOTIABLES, todayEvents, capacity, dow, examTargets, targetKey, targetLabel: date.toLocaleDateString([], { weekday: "long", month: "short", day: "numeric" }) };
  },

  systemPrompt() {
    return `You are a planning assistant for a Year 10 student juggling school, several small businesses, and personal development goals. You turn a static list of open tasks into a specific, realistic plan for ONE TARGET DAY only.
...
- Never invent tasks that aren't drawn from the provided open task list.`;
  ...
  async generateRationale(ctx, onToken) {
    const capacityLine = ctx.capacity.sampleSize >= 2
      ? `Historical capacity for this weekday: averaged ${ctx.capacity.avgCompletedMinutes} completed minutes across ${ctx.capacity.sampleSize} comparable days (${Math.round(ctx.capacity.avgRatio * 100)}% average completion rate).`
    ...
    const userPrompt = `Target planning date: ${ctx.targetLabel} (${ctx.targetKey})
Open tasks available: ${ctx.openTasks.length} (ventures: ${[...new Set(ctx.openTasks.map(t => t.venture))].join(", ") || "none"})
Target-day fixed rhythm blocks: ${ctx.rhythmToday.join(", ") || "none"}
Current timetable phase: ${ctx.phase ? `${ctx.phase.label} — ${ctx.phase.focus}` : "none"}
Target-day calendar events: ${ctx.todayEvents.join(", ") || "none"}
${capacityLine}
${examLine}
...
Briefly explain your reasoning before the plan.`;
  ...
  async generate({ silent = true, onRationaleToken = null, targetDate = new Date() } = {}) {
    if (!(await ClaudeClient.hasKey())) return { ok: false, reason: "no-key" };

    await this.archiveYesterdayIfNeeded();

    const ctx = await this.buildContext(targetDate);
    if (!ctx.openTasks.length) return { ok: false, reason: "no-tasks" };

    const rationale = await this.generateRationale(ctx, onRationaleToken);

    const capacityLine = ctx.capacity.sampleSize >= 2
      ? `Historical capacity for this weekday: you've completed an average of ${ctx.capacity.avgCompletedMinutes} minutes across ${ctx.capacity.sampleSize} comparable days (${Math.round(ctx.capacity.avgRatio * 100)}% average completion rate). Plan close to this, not above it.`
    ...
    const userPrompt = `Target planning date: ${ctx.targetLabel} (${ctx.targetKey})
Open task library (pick from these only):\n${JSON.stringify(ctx.openTasks, null, 2)}
...
Generate the plan for the exact target date above.`;
    ...
    try {
      ...
      if (ctx.targetKey > this.todayKey()) {
        await chrome.storage.local.set({ hq_daily_plan_future_v1: { date: ctx.targetKey, tasks, preMortem, rationale: rationale || "", preparedAt: Date.now() } });
      } else {
        await chrome.storage.local.set({
          hq_daily_plan_date: ctx.targetKey,
      ...
      this.fireWebhook({ date: ctx.targetKey, taskCount: tasks.length, plannedMinutes: plannedTotal });

      return { ok: true, tasks, preMortem, rationale };
    } catch (e) {
      console.error("Daily plan generation failed:", e);
      if (!silent && typeof Wallpaper !== "undefined") {
        Wallpaper.toast(`Couldn't generate the plan: ${e.message}`);
      }
  ...
  async init() {
    const storedPlan = await this.getStoredPlan();
    ...
    this.wireReadAloud();
    this.render();
    PageScheduler.register("daily-plan-rollover", 60000, async () => {
      const plan = await this.getStoredPlan();
      if (plan.promoted) this.render();
    });
