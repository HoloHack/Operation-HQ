...
const SpotifyClient = {
  ...
  async isConnected() {
    return !!(await CredentialVault.getSecret("hq_spotify_refresh_token"));
  },
  ...
  async storeTokens(data) {
    ...
    await CredentialVault.set(toStore);
  },

  async refreshAccessToken() {
    const clientId = await this.getClientId();
    const { hq_spotify_refresh_token } = await CredentialVault.get("hq_spotify_refresh_token");
  ...
  async getValidAccessToken() {
    const { hq_spotify_access_token, hq_spotify_token_expires_at } = await CredentialVault.get(["hq_spotify_access_token", "hq_spotify_token_expires_at"]);
    ...
    const { hq_spotify_access_token: fresh } = await CredentialVault.get("hq_spotify_access_token");
    return fresh;
  },

  async disconnect() {
    await CredentialVault.remove(["hq_spotify_access_token", "hq_spotify_refresh_token", "hq_spotify_token_expires_at"]);
  },
