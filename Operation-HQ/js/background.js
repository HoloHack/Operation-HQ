...
chrome.storage.onChanged.addListener((changes, area) => {
  ...
  if (changes.hq_deepwork_active || changes.hq_deepwork_sites) applyDeepWorkRules();
  if (changes.hq_deepwork_active || changes.hq_deepwork_js_disabled_sites) applyContentSettingsRules();
});
...
chrome.alarms.onAlarm.addListener((alarm) => {
  if (alarm.name === "hq_wallpaper_rotate") {
    chrome.storage.local.set({ hq_wallpaper_cached: null });
  }
});
...
chrome.alarms.onAlarm.addListener(safely("Nightly plan", async (alarm) => {
  if (alarm.name === "hq_daily_plan_nightly") {
    // Generates *tomorrow's* plan conceptually, but since DailyPlanner keys
    // off "today" and this fires at 23:55, by the time anyone opens a new
    // tab it'll already be past midnight for anyone up that late, or the
    // newtab-boot fallback picks it up the next morning either way.
    if (await ClaudeClient.hasKey()) {
      await DailyPlanner.generate({ silent: true });
    }
  }
}));
