...
// Heavy or network-backed panels initialize on first use. This keeps every new
// tab's critical path small and prevents a broken integration from breaking the
// clock, wallpaper, tasks or dock. Each initializer still runs at most once.
const LazyFeatures = {
  started: new Map(),
  definitions: {
    "notes-flyout": ["Notes", () => NotesEditor.init()],
    "gmail-flyout": ["Gmail", () => Gmail.init()],
    "spotify-flyout": ["Spotify", async () => {
      await SpotifyUI.init();
      if (await SpotifyClient.isConnected()) await SpotifyPlayer.connect();
    }],
    "stats-flyout": ["Weekly stats", () => WeeklyStats.init()],
    "venture-dash-flyout": ["Venture dashboard", () => VentureDashboard.init()],
    "radar-flyout": ["Idea radar", () => IdeaRadar.init()],
    "optimizer-flyout": ["Browser workspaces", () => ChromeOptimizer.init()],
    "gleam-flyout": ["Gleam social confidence lab", () => GleamHQ.init()],
  ...
  async ensure(panelId) {
    ...
    if (!this.started.has(panelId)) {
      const [label, initializer] = this.definitions[panelId];
...
function applyStaticIcons() {
  ...
  if (build) build.textContent = `${chrome.runtime.getManifest().version} · Resilient Surface`;
  const map = {
  ...
  async function peekTextFor(panelId) {
    switch (panelId) {
      ...
      case "bookmarks-flyout":
        return await Bookmarks.peekSummary();
      case "srs-flyout": {
...
function wireBookmarkSorter() {
  document.getElementById("sort-bookmarks-btn").onclick = () => Bookmarks.sortAll();
  document.getElementById("preview-bookmarks-btn").onclick = () => Bookmarks.preview();
  document.getElementById("undo-bookmarks-btn").onclick = () => Bookmarks.undo();
  document.getElementById("sort-inbox-btn").onclick = () => Bookmarks.sortInboxOnly();
  document.getElementById("refine-uncategorized-btn").onclick = () => Bookmarks.refineUncategorized();
...
function wireApiKeys() {
  ...
  chrome.storage.local.get(Object.values(fields)).then(s => {
    Object.entries(fields).forEach(([id, key]) => { document.getElementById(id).value = s[key] || ""; });
  });
  Object.entries(fields).forEach(([id, key]) => {
    let debounce;
    document.getElementById(id).oninput = (e) => {      clearTimeout(debounce);
...
async function boot() {
  ...
  await BootDiagnostics.run("Clock", () => updateClock());
  ...
  await BootDiagnostics.run("Bookmarks", () => Bookmarks.init());
  await BootDiagnostics.run("Assessment intake detector", () => AssessmentIntake.boot());
  ...
  await BootDiagnostics.run("Mods", () => Mods.init());
  await BootDiagnostics.run("Capture inbox", () => Capture.init());
  await BootDiagnostics.run("Forgotten bookmark", () => ForgottenBookmark.init());
  await BootDiagnostics.run("Idea vault", () => IdeaVault.init());
  await BootDiagnostics.run("Professional view", () => ProfessionalView.init());
  await BootDiagnostics.run("Deep Work", () => DeepWork.init());
  await BootDiagnostics.run("Language practice", () => SRS.init());
