...
const AssessmentIntake = {
  ...
  async accept(item) {
    ...
    Calendar.events[analysis.dueDate] = Calendar.events[analysis.dueDate] || [];
    if (!Calendar.events[analysis.dueDate].includes(deadlineEvent)) Calendar.events[analysis.dueDate].push(deadlineEvent);
  ...
  async undoAccept(item) {
    ...
    await Assignments.save();
    if (item.deadlineEvent && Calendar.events[item.analysis.dueDate]) {
      Calendar.events[item.analysis.dueDate] = Calendar.events[item.analysis.dueDate].filter(value => value !== item.deadlineEvent);
      if (!Calendar.events[item.analysis.dueDate].length) delete Calendar.events[item.analysis.dueDate];
      await chrome.storage.local.set({ hq_calendar_events: Calendar.events });
      await Calendar.render();
    }
    item.state = "pending";
