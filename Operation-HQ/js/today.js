...
const Today = {
  ...
  async capture(text, destination) {
    if (destination === "task") {
      ...
      Calendar.events[key] = [...(Calendar.events[key] || []), text];
      ...
      this.showUndo("Added to today’s Calendar.", async () => { Calendar.events = before; await chrome.storage.local.set({ hq_calendar_events: before }); Calendar.render(); });
    }
    await this.render();
  },
  ...
  async handleFollowup(event) {
    ...
    if (!item) return;
    if (button.dataset.followupAction === "open" && item.threadId) Gmail.openThread(item.threadId);
