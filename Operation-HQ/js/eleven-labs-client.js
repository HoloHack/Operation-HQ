...
// endpoint, same honest pattern as claude-client.js: key lives in
// chrome.storage.local (plaintext, single-user, unpacked-extension
// tradeoff — same disclosure applies, never publish this build with a
// key still in it), gates cleanly to a no-op when no key is set.
//
...
const ElevenLabsClient = {
  ...
  async getKey() {
    const { hq_key_elevenlabs } = await chrome.storage.local.get("hq_key_elevenlabs");
    return hq_key_elevenlabs || "";
  },
