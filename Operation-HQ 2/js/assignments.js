...
const Assignments = {
  ...
  async applyPlan() {
    ...
    const entries = this.draft.map(entry => ({ dateKey: entry.dateKey, text: `Assignment · ${entry.subject} · ${entry.s}–${entry.e} · ${entry.title}` }));
    const result = await CalendarRepository.addLegacyMany(entries);
    CalendarRepository.syncCalendar(result);
    if (result.inverse.length) await chrome.storage.local.set({ [this.UNDO_KEY]: { inverse: result.inverse, createdAt: Date.now() } });
    else await chrome.storage.local.remove(this.UNDO_KEY);
    document.getElementById("assignment-plan-undo").disabled = !result.inverse.length;
    const count = result.inverse.length;
    await Calendar.render();
    await Today?.render?.();
    this.cancelPlan();
    this.status(count ? `${count} reviewed assignment session${count === 1 ? "" : "s"} added. Undo remains available.` : "Those assignment sessions were already on the calendar; nothing was duplicated.");
  },

  async undoPlan() {
    const saved = await chrome.storage.local.get(this.UNDO_KEY);
    const undo = saved[this.UNDO_KEY];
    if (!Array.isArray(undo?.inverse)) return;
