...
const AmbientCompositor = {
  ...
  profile() {
    ...
    // Auto mode is deliberately conservative. A new-tab surface should not
    // reserve a 60 fps graphics budget merely because a laptop reports eight
    // logical cores; Ultra remains available as an explicit choice.
    const auto = saveData || cores <= 4 || memory <= 4
      ? "efficient"
      : cores >= 12 && memory >= 16
        ? "ultra"
        : "balanced";
  ...
  releaseDrawingBuffer() {
    if (!this.canvas || !this.gl) return;
    // Canvas backing stores can occupy tens of megabytes on high-DPI screens.
    // A hidden/off compositor keeps only a one-pixel buffer and restores its
    // adaptive resolution when the tab becomes visible again.
  ...
  samplePerformance(time) {
    ...
    if (this.settings.adaptive && this.settings.quality !== "off" && this.renderedFrames > 90) {
      if (this.measuredFps < target * 0.72 && this.renderScale > 0.36) {
        ...
        this.resize();
      } else if (this.measuredFps < target * 0.72) {
        this.lowSamples += 1;
      ...
      if (this.lowSamples >= 2 && this.frameInterval < 1000 / 30) {
        this.frameInterval = 1000 / 30;
        document.body.dataset.visualBudget = "efficient";
        this.updateStatus("Procedural atmosphere automatically moved to a smooth 30 fps budget; interface responsiveness is protected.");
      }
    }
    this.updateStatus();
  },
  ...
  applyProfile() {
    ...
    document.body.dataset.visualBudget = !profile.enabled ? "css" : profile.fps >= 60 ? "ultra" : profile.fps >= 45 ? "balanced" : "efficient";
  ...
  async startRenderer() {
    ...
    try {
      ...
      const powerPreference = this.settings.quality === "ultra" ? "high-performance" : "low-power";
      this.gl = this.canvas.getContext("webgl2", { alpha: true, antialias: false, depth: false, stencil: false, powerPreference, premultipliedAlpha: true });
