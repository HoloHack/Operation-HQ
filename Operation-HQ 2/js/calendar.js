...
const Calendar = {
  ...
  async _init() {
    const saved = await chrome.storage.local.get(["hq_calendar_events", this.DETAILS_KEY, CalendarRepository.IDS_KEY, "hq_calendar_view_v2", "hq_calendar_undo_v2"]);
    ...
    this.entryIds = this.validDetails(saved[CalendarRepository.IDS_KEY]) ? saved[CalendarRepository.IDS_KEY] : {};
  ...
  bindStorage() {
    ...
    chrome.storage.onChanged.addListener((changes, area) => {
      ...
      if (changes[CalendarRepository.IDS_KEY]) this.entryIds = this.validDetails(changes[CalendarRepository.IDS_KEY].newValue) ? changes[CalendarRepository.IDS_KEY].newValue : {};
  ...
  recordsForDate(dateKey, { applyFilter = true } = {}) {
    ...
    legacy.forEach((text, index) => {
      if (!consumed[index]) records.push({ id: this.entryIds[dateKey]?.[index] || `legacy-${dateKey}-${index}`, title: text, date: dateKey, occurrenceDate: dateKey, displayText: text, allDay: true, category: "personal", legacy: true, legacyIndex: index, legacyId: this.entryIds[dateKey]?.[index] || "" });
    });
    const query = applyFilter ? this._query : "";
    return records
  ...
  openEditor(record, dateKey = this.selectedDate) {
    ...
    form.dataset.legacyId = isLegacy ? String(record.legacyId || record.id || "") : "";
  ...
  async saveEditor() {
    ...
    if (form.dataset.legacyDate) {
      ...
      actions.push({ type: "removeLegacy", dateKey: form.dataset.legacyDate, id: form.dataset.legacyId, index, text: this.events[form.dataset.legacyDate]?.[index] || "" });
    }
    actions.push({ type: "upsertEvent", event });
    const result = await CalendarRepository.transaction(actions);
    CalendarRepository.syncCalendar(result);
    await this.storeUndo("Event change", result.inverse);
  ...
  async deleteEditorEvent() {
    ...
    if (id && this.details[id]) {
      actions = [{ type: "removeEvent", id }];
    } else if (form.dataset.legacyDate) {
      const index = Number(form.dataset.legacyIndex);
      actions = [{ type: "removeLegacy", dateKey: form.dataset.legacyDate, id: form.dataset.legacyId, index, text: this.events[form.dataset.legacyDate]?.[index] || "" }];
    }
    const result = await CalendarRepository.transaction(actions);
    CalendarRepository.syncCalendar(result);
    await this.storeUndo("Event removed", result.inverse);
    this.closeEditor();
    await this.render();
    Wallpaper?.toast?.("Calendar event removed.");
  },

  async storeUndo(label, inverse) {
    if (!Array.isArray(inverse) || !inverse.length) {
  ...
  async undoLastChange() {
    ...
    if (!undo || !Array.isArray(undo.inverse)) {
      await chrome.storage.local.remove("hq_calendar_undo_v2");
      this.el("cal-undo").classList.add("hidden");
      Wallpaper?.toast?.("That legacy undo snapshot was retired to protect newer calendar changes.");
      return;
    }
    const result = await CalendarRepository.undo(undo.inverse);
    CalendarRepository.syncCalendar(result);
    await chrome.storage.local.remove("hq_calendar_undo_v2");
    this.el("cal-undo").classList.add("hidden");
    this.closeEditor();
  ...
  async quickAdd() {
    ...
    this.materialize(event);
    const result = await CalendarRepository.transaction([{ type: "upsertEvent", event }]);
    CalendarRepository.syncCalendar(result);
    await this.storeUndo("Quick add", result.inverse);
    this.selectedDate = event.date;
    this.viewDate = this.fromKey(event.date);
    input.value = "";
