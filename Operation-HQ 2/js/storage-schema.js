...
const StorageSchema = {
  ...
  migrations: {
    ...
    // Cross-context integrity: stable calendar entry ids support exact undo,
    // Context Bus revisions expose ordering, and unsafe whole-calendar undo
    // snapshots from older builds are retired rather than replayed.
    9(state) {
      const patch = {};
      const entryIds = state.hq_calendar_entry_ids_v1;
      if (entryIds != null && (!entryIds || typeof entryIds !== "object" || Array.isArray(entryIds))) patch.hq_calendar_entry_ids_v1 = {};
      const revision = Number(state.hq_calendar_revision_v1);
      if (!Number.isFinite(revision) || revision < 0) patch.hq_calendar_revision_v1 = 0;
      if (state.hq_context && typeof state.hq_context === "object" && !Array.isArray(state.hq_context) && !Number.isFinite(Number(state.hq_context.revision))) {
        patch.hq_context = { ...state.hq_context, revision: 0 };
      }
